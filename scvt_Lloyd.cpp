...
int main(int argc, char **argv){
    ...
    string t_names[num_timers] = {"Cumulative Time", "Global Time", "Final Triangulation", "Initialization"};
    for(i = 0; i < num_timers; i++){
        timers[i] = mpi_timer(t_names[i]);
    }
    timers[1].start(); // Global Time Timer
    timers[3].start(); // Initialization Time Timer
...
*/
            ...
            if(id==0)
            {
                ...
                if(it_bisect==num_bisections)
                    itrPolarFile << it << " " << glob_l2 << " " << fval << " " << grad_normPolar << " "
    ...
    if(id == 0){
        ofstream end_pts("end_points.dat."+fileName);
        ofstream pt_dens("point_density.dat."+fileName);
        //ofstream bdry_pts("boundary_points.dat");
        for(point_itr = points.begin(); point_itr != points.end(); ++point_itr){
            end_pts << (*point_itr) << endl;
            pt_dens << density((*point_itr)) << endl;
...
/*  //Bisect all edges of all triangles to give an extra point set at the end, SaveVertices
    if(num_bisections>0)
        bisectTriangulation(flags, world, my_regions, all_triangles, regions, points, 1, "bisected_points.dat."+fileName);
*/
